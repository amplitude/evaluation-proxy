...
class ProjectRunnerTest {
    ...
    @Test
    fun `test start, no state initial load, success`(): Unit =
        runBlocking {
    ...
    @Test
    fun `test start, with initial state, add and remove deployment, success`(): Unit =
        runBlocking {
            val projectApi = mockk<ProjectApi>()
            val deploymentLoader = mockk<DeploymentLoader>()
            val deploymentStorage =
                spyk<DeploymentStorage>(
    ...
    @Test
    fun `test start, deployments api failure, does not throw`(): Unit =
        runBlocking {
            val projectApi = mockk<ProjectApi>()
            val deploymentLoader = mockk<DeploymentLoader>()
            val deploymentStorage = spyk<DeploymentStorage>(InMemoryDeploymentStorage())
