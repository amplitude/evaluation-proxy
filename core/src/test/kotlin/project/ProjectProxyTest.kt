...
class ProjectProxyTest {
    ...
    @Test
    fun `test get flag configs, null deployment, unauthorized`(): Unit = runBlocking {
    ...
    @Test
    fun `test get flag configs, with deployment, success`(): Unit = runBlocking {
        val deployment = deployment("deployment")
        val flag = flag("flag")
        val assignmentTracker = mockk<AssignmentTracker>()
        val deploymentStorage = InMemoryDeploymentStorage().apply {
    ...
    @Test
    fun `test get cohort, null cohort id, not found`(): Unit = runBlocking {
        val assignmentTracker = mockk<AssignmentTracker>()
        val deploymentStorage = InMemoryDeploymentStorage()
        val cohortStorage = InMemoryCohortStorage()
        val projectProxy = ProjectProxy(
    ...
    @Test
    fun `test get cohort, with cohort id, success`(): Unit = runBlocking {
        val cohort = cohort("a")
        val assignmentTracker = mockk<AssignmentTracker>()
        val deploymentStorage = InMemoryDeploymentStorage()
        val cohortStorage = InMemoryCohortStorage().apply {
            putCohort(cohort)
        }
        val projectProxy = ProjectProxy(
            project,
    ...
    @Test
    fun `test get cohort, with cohort id and last modified, success`(): Unit = runBlocking {
    ...
    @Test
    fun `test get cohort, with cohort id and last modified, not changed`(): Unit = runBlocking {
        val cohort = cohort("a", 100)
        val assignmentTracker = mockk<AssignmentTracker>()
        val deploymentStorage = InMemoryDeploymentStorage()
        val cohortStorage = InMemoryCohortStorage().apply {
    ...
    @Test
    fun `test get cohort, with cohort id and max cohort size, success`(): Unit = runBlocking {
        val deployment = deployment("deployment")
    ...
    @Test
    fun `test get cohort, with cohort id and max cohort size, too large`(): Unit = runBlocking {
        val cohort = cohort("a", 100, 100)
        val assignmentTracker = mockk<AssignmentTracker>()
        val deploymentStorage = InMemoryDeploymentStorage()
        val cohortStorage = InMemoryCohortStorage().apply {
    ...
    @Test
    fun `test get cohort memberships for group, null deployment, unauthorized`(): Unit = runBlocking {
        val assignmentTracker = mockk<AssignmentTracker>()
        val deploymentStorage = InMemoryDeploymentStorage()
        val cohortStorage = InMemoryCohortStorage()
        val projectProxy = ProjectProxy(
    ...
    @Test
    fun `test get cohort memberships for group, with deployment and group type, null group name, bad request`(): Unit = runBlocking {
        val deployment = deployment("deployment")
        val assignmentTracker = mockk<AssignmentTracker>()
        val deploymentStorage = InMemoryDeploymentStorage()
        val cohortStorage = InMemoryCohortStorage()
    ...
    @Test
    fun `test get cohort memberships for group, with deployment and group name, null group type, bad request`(): Unit = runBlocking {
        val deployment = deployment("deployment")
        val assignmentTracker = mockk<AssignmentTracker>()
        val deploymentStorage = InMemoryDeploymentStorage()
        val cohortStorage = InMemoryCohortStorage()
    ...
    @Test
    fun `test get cohort memberships for group, with deployment group name and group type, success`(): Unit = runBlocking {
        val deployment = deployment("deployment")
        val cohort = cohort("a")
        val flag = flag("flag", setOf("a"))
        val assignmentTracker = mockk<AssignmentTracker>()
    ...
    @Test
    fun `test evaluate, null deployment, unauthorized`(): Unit = runBlocking {
        val assignmentTracker = mockk<AssignmentTracker>()
        val deploymentStorage = InMemoryDeploymentStorage()
        val cohortStorage = InMemoryCohortStorage()
        val projectProxy = ProjectProxy(
    ...
    @Test
    fun `test evaluate, with deployment, null user, success`(): Unit = runBlocking {
        val deployment = deployment("deployment")
        val assignmentTracker = mockk<AssignmentTracker>()
        val deploymentStorage = InMemoryDeploymentStorage()
        val cohortStorage = InMemoryCohortStorage()
    ...
    @Test
    fun `test evaluate, with deployment and flag keys, success`(): Unit = runBlocking {
        val cohort = cohort("a")
        val flag = flag("flag", setOf("a"))
        val deployment = deployment("deployment")
        val assignmentTracker = mockk<AssignmentTracker>()
