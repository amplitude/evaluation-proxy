...
class DeploymentRunnerTest {
    ...
    @Test
    fun `start, stop, load deployment called once, success`(): Unit = runBlocking {
    ...
    @Test
    fun `start, delay, periodic loaders run, success`(): Unit = runBlocking {
    ...
    @Test
    fun `start, load deployment throws, does not throw, pollers run`(): Unit = runBlocking {
        val cohortIds = setOf("a")
        val flag = flag(cohortIds = cohortIds)
        val configuration = Configuration(
            flagSyncIntervalMillis = 50,
    ...
    @Test
    fun `start get all flags throws, does not throw`(): Unit = runBlocking {
        val configuration = Configuration(
