...
class DeploymentLoaderTest {
    ...
    @Test
    fun `load flags without cohorts, cohorts not loaded, success`(): Unit = runBlocking {
    ...
    @Test
    fun `load flags with cohorts, cohorts are loaded, success`(): Unit = runBlocking {
        val api = mockk<DeploymentApi>()
        val storage = spyk(InMemoryDeploymentStorage())
        val cohortLoader = mockk<CohortLoader>()
    ...
    @Test
    fun `existing flags state, some flags removed, success`(): Unit = runBlocking {
        val existingFlagKey = "existing"
        val existingFlag = flag(existingFlagKey)
        val api = mockk<DeploymentApi>()
        val storage = spyk(InMemoryDeploymentStorage().apply {
    ...
    @Test
    fun `getFlagConfigs fails, throws`(): Unit = runBlocking {
        val api = mockk<DeploymentApi>()
        val storage = spyk(InMemoryDeploymentStorage())
        val cohortLoader = mockk<CohortLoader>()
        val cohortIds = setOf("a", "b")
    ...
    @Test
    fun `removeFlag fails, throws`(): Unit = runBlocking {
        val existingFlagKey = "existing"
        val existingFlag = flag(existingFlagKey)
        val api = mockk<DeploymentApi>()
        val storage = spyk(InMemoryDeploymentStorage().apply {
    ...
    @Test
    fun `loadCohorts fails, throws`(): Unit = runBlocking {
        val api = mockk<DeploymentApi>()
        val storage = spyk(InMemoryDeploymentStorage())
        val cohortLoader = mockk<CohortLoader>()
        val cohortIds = setOf("a", "b")
    ...
    @Test
    fun `putFlag fails, throws`(): Unit = runBlocking {
        val api = mockk<DeploymentApi>()
        val storage = spyk(InMemoryDeploymentStorage())
        val cohortLoader = mockk<CohortLoader>()
        val cohortIds = setOf("a", "b")
