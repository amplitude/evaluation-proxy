...
class CacheTest {
    ...
    @Test
    fun `test get no entry`() = runBlocking {
    ...
    @Test
    fun `test set and get`() = runBlocking {
        val cache = Cache<Int, Int>(4)
        cache.set(0, 0)
        val value = cache.get(0)
    ...
    @Test
    fun `test least recently used entry is removed`() = runBlocking {
        val cache = Cache<Int, Int>(4)
        repeat(4) { i ->
            cache.set(i, i)
        }
    ...
    @Test
    fun `test first set then get entry is not removed`() = runBlocking {
        val cache = Cache<Int, Int>(4)
        repeat(4) { i ->
            cache.set(i, i)
        }
    ...
    @Test
    fun `test first set then re-set entry is not removed`() = runBlocking {
        val cache = Cache<Int, Int>(4)
        repeat(4) { i ->
            cache.set(i, i)
        }
    ...
    @Test
    fun `test first set then re-set with different value entry is not removed`() = runBlocking {
        val cache = Cache<Int, Int>(4)
        repeat(4) { i ->
            cache.set(i, i)
        }
