...
class EvaluationProxyTest {
    ...
    @Test
    fun `test start, no projects, nothing happens`(): Unit = runBlocking {
        val projectStorage = spyk(InMemoryProjectStorage())
        val projectConfigurations = listOf<ProjectConfiguration>()
        val evaluationProxy = spyk(EvaluationProxy(
    ...
    @Test
    fun `test start, with project, storage loaded and proxy started`(): Unit = runBlocking {
        val projectStorage = spyk(InMemoryProjectStorage())
        val project = project("1")
        val projectConfigurations = listOf(
    ...
    @Test
    fun `test start, stored but no longer configured project deleted`(): Unit = runBlocking {
        val projectStorage = spyk(InMemoryProjectStorage().apply {
            putProject("2")
        })
        val project = project("1")
        val projectConfigurations = listOf(
        ...
        val deploymentStorage = mockk<DeploymentStorage>().apply {
        ...
        val cohortStorage = mockk<CohortStorage>().apply {
