...
class CohortLoaderTest {
    ...
    @Test
    fun `load cohorts, success`(): Unit = runBlocking {
    ...
    @Test
    fun `load cohorts, simultaneous loading of the same cohorts, only downloads once per cohort`(): Unit = runBlocking {
        val cohortA = Cohort("a", "User", 1, 100, setOf("1"))
        val cohortB = Cohort("b", "User", 1, 100, setOf("1"))
        val cohortC = Cohort("c", "User", 1, 100, setOf("1"))
        val api = mockk<CohortApi>()
    ...
    @Test
    fun `load cohorts, failure, failed cohort not stored, does not throw`(): Unit = runBlocking {
        val cohortA = Cohort("a", "User", 1, 100, setOf("1"))
        val cohortC = Cohort("c", "User", 1, 100, setOf("1"))
        val api = mockk<CohortApi>()
        val storage: CohortStorage = spyk(InMemoryCohortStorage())
